package com.epam.drill.auto.test.agent.penetration.http.selenium

import com.epam.drill.auto.test.agent.penetration.http.selenium.DevToolsClientThreadStorage.crhmT
...
class ChromeDevTool {
    ...
    fun connectToDevTools(capabilities: Map<*, *>?) = capabilities?.let { cap ->
    ...
    private fun doDevToolsRequest(debuggerURL: String): HttpURLConnection {
        ...
        logger.info { "Try to get chrome info [${obj.path}]" }
...
class ChromeDevToolWs(uri: URI, private val cdl: CountDownLatch) : WebSocketClient(uri) {
    ...
    override fun onMessage(message: String) = runCatching {
        ...
        when (step) {
            ...
            else -> logger.info { "unprocessed step" }
        }
    ...
    private fun sendAttachRequest(result: String) {
        logger.info { "DevTools session created" }
